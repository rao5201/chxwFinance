...
const userBehaviorTracker = {
  ...
  cleanupSessions: () => {
    const now = Date.now();
    const timeout = monitoringConfig.userBehavior.sessionTimeout;
...
const anomalyDetector = {
  ...
  detectAnomaly: (metricName, value) => {
    ...
    if (monitoringConfig.anomalyDetection.sensitivity === 'low') {
      threshold = 3;
    } else if (monitoringConfig.anomalyDetection.sensitivity === 'high') {
      threshold = 1.5;
    }
...
const alertManager = {
  ...
  checkRules: (metrics) => {
    ...
    for (const rule of monitoringConfig.alerting.rules) {
...
const metricsMiddleware = (req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    // 检测异常
    if (duration > monitoringConfig.anomalyDetection.thresholds.responseTime) {
